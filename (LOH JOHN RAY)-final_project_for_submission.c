...
--------------------------------------------------------------------------
       ...
       A user-friendly companion for introductory courses to Calculus.
 
 Here's a quick breakdown:
 
        (i) A definite integral is defined as the area beneath a function.
        
       (ii) This program uses Trapezoidal Rule to calculate the integral:
       
             x=a
              /            dx  /                                          \\
              | f(x) dx =  -- | f(x_0) + 2f(x_1) + ... + 2f(x_n-1) f(x_n)  | 
              /             2  \\                                          /
             x=b
            
                        b - a 
         where dx =     ----- ,  and x_i = x + i*dx
                          n
...
//(*func) gets passes to trapezoidal_recursive, indirection unary operator
